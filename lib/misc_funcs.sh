...
function load_config() {
	output_section "Checking Erlang versions"

	local custom_config_file="${build_path}/erlang_buildpack.config"

	# Source for default versions file from buildpack first
	source "${build_pack_path}/erlang_buildpack.config"

	if [ -f $custom_config_file ]; then
		source $custom_config_file
	else
		output_line "WARNING: erlang_buildpack.config wasn't found in the app"
		output_line "Using default config from Erlang buildpack"
	fi
